...
export const DataroomPanel = memo(function DataroomPanel() {
  const isCollapsed = useUIStore((state) => state.dataroomsCollapsed);
  const setDataroomsCollapsed = useUIStore(
    (state) => state.setDataroomsCollapsed,
  );
    ...
    <section className="rounded-3xl border border-border bg-panel/90 p-5 shadow-card">
      <div className="flex items-center justify-between gap-3">
        <button
          ...
          onClick={() => setDataroomsCollapsed(!isCollapsed)}
          aria-label={isCollapsed ? "Expand data rooms" : "Collapse data rooms"}
